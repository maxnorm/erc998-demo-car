...
/**
 * Sell the car
 * @param carToken - the token id of the car to sell
 * @param to - the address to sell the car to
 */
...
/**
 * Fuel the car
 * @param carToken - the token id of the car to fuel
 * @param from - the address to fuel the car from
 */
async function fuelCar(carToken: string, from: string): Promise<void> {
  console.log(line);
  console.log(`| Fueling Car #${carToken} from ${from}`);
  console.log(line);

  await waitForInput("Before we can fuel the car, we need to mint some fuel");
  const fuelToken = await mintFuel(await bob.getAddress());


  const fuelAmount = 100;
  const fuelAmountWei = BigInt(fuelAmount) * BigInt(10**18);
  await waitForInput(`Now that we have ${Number(fuelToken) / 10**18} L fuel, let's fuel the car with ${fuelAmount}L fuel...`);
  
  const approveTx = await fuel.connect(bob).approve(carAddress, fuelAmountWei);
  await approveTx.wait();

  console.log(line);
  console.log(`| Bob approved Car contract to claim ${fuelAmount}L of fuel`);
  console.log(`|`);
  
  // Then use getERC20 to add the fuel to the car
  const tx = await car.connect(bob).getERC20(await bob.getAddress(), carToken, fuelAddress, fuelAmountWei);
  const receipt = await tx.wait();

  console.log(`| Transaction hash: ${tx.hash}`);
  console.log(`| Gas used: ${receipt.gasUsed.toString()}`);
  console.log(`| Fuel successfully transferred to Car #${carToken} with ${fuelAmount} fuel`);
  console.log(line);
...
async function main(): Promise<void> {
  ...
  console.log(line);
  console.log("|");
  console.log("| 🎉 Congrats to Bob");
  console.log("| He now owns a car with 1 engine, 4 wheels, and 1 full fuel tank!");
  console.log("|");
  console.log(line);
  console.log(carArt);
}
